...
export default class CoinInfo extends Component {
  ...
  chartCall = (days) => {
    fetch(
      `https://api.coingecko.com/api/v3/coins/${this.props.match.params.id}/market_chart?vs_currency=usd&days=${days}&interval=daily`
    )
  ...
  coinCall = () => {
    fetch(
      `https://api.coingecko.com/api/v3/coins/${this.props.match.params.id}?tickers=false&market_data=true`
    )
      .then((response) => response.json())
      .then((resData) => {
            ...
            <div className="rightSections">
              ...
              <p>
                {this.convertToInternationalCurrencySystem(
                  this.state?.coinData?.market_data?.market_cap?.usd
                )}
              </p>
            </div>
            <div className="rightSections">
              <p>Current Price</p>
              <p>{`$${this.convertToInternationalCurrencySystem(
                this.state?.coinData?.market_data?.current_price?.usd
              )}`}</p>
            </div>
            <div className="rightSections">
              <p>All Time High</p>
              <p>{`$${this.convertToInternationalCurrencySystem(
                this?.state?.coinData?.market_data?.ath?.usd
              )}`}</p>
            </div>
            <div className="rightSections">
              <p>24h Change</p>
